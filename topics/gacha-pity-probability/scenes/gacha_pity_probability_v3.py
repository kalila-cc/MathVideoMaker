...
TARGET_DURATIONS_V3 = {
    "ThreeNumbersHookV3": 21.195,
    "CohortAfterEachPull": 42.269,
    "PullFiftyThreeQuestionsV3": 45.417,
    "ConditionalProbabilityCurveFull": 22.906,
    "FirstGoldPmfCurveFull": 32.456,
    "CumulativeCdfCurveFull": 29.112,
    "CompactCurveRelationship": 30.175,
    "HardPityCurveDashboard": 34.134,
    "IntegratedRateVsHardPity": 33.289,
    "NonUniquePerPullCurves": 28.159,
    "FiveStarThenUpIdentityV3": 26.042,
    "UpWaitingTwoPeaksV3": 41.491,
    "CapturingRadianceBasicsV3": 32.072,
    "CapturingRadianceHistoryV3": 27.785,
    "ProbabilityStateConclusionV3": 34.156,
}

LEFT_CENTER = np.array([-2.52, -0.30, 0.0])
RIGHT_CENTER = np.array([4.42, -0.30, 0.0])
LEFT_WIDTH = 8.42
RIGHT_WIDTH = 4.42
CONTENT_HEIGHT = 5.22
DIM_GOLD = "#A37B2C"
...
def compact_chart_shell(
    ...
    *,
    ...
    box = panel(3.95, 2.34, fill=PANEL, edge=PANEL_EDGE).move_to(center)
...
def cohort_grid(center: np.ndarray, count: int = 1000, columns: int = 40) -> VGroup:
    ...
    for index in range(count):
        ...
        row = index // columns
        point = center + RIGHT * (column - (columns - 1) / 2) * 0.116
        point += UP * ((rows - 1) / 2 - row) * 0.116
...
class ThreeNumbersHookV3(Scene):
    def construct(self) -> None:
        set_scene_background(self)
        title = scene_title("0.6%、1.6%和90抽，分别在回答什么？")
        ...
        finish_to(self, TARGET_DURATIONS_V3["ThreeNumbersHookV3"])


class CohortAfterEachPull(Scene):
    def construct(self) -> None:
        set_scene_background(self)
        title = scene_title("1万人一起抽：每一抽后，人群怎样变化？", "固定0.6%、没有保底的教学对照")
        self.play(FadeIn(title, shift=DOWN * 0.12), run_time=0.8)
        ...
        finish_to(self, TARGET_DURATIONS_V3["CohortAfterEachPull"])


class PullFiftyThreeQuestionsV3(Scene):
    def construct(self) -> None:
        set_scene_background(self)
        title = scene_title("“第50抽的概率”到底在问什么？", "三个答案来自三个不同的分母")
        self.play(FadeIn(title, shift=DOWN * 0.12), run_time=0.8)
        ...
        finish_to(self, TARGET_DURATIONS_V3["PullFiftyThreeQuestionsV3"])


class ConditionalProbabilityCurveFull(Scene):
    def construct(self) -> None:
        set_scene_background(self)
        title = scene_title("第一条：这一抽会不会出金？", "从零垫开始，研究第一次出五星要等多少抽")
        self.play(FadeIn(title, shift=DOWN * 0.12), run_time=0.8)
        ...
        finish_to(self, TARGET_DURATIONS_V3["ConditionalProbabilityCurveFull"])


class FirstGoldPmfCurveFull(Scene):
    def construct(self) -> None:
        set_scene_background(self)
        title = scene_title("第二条：第一次出金落在哪一抽？")
        self.play(FadeIn(title, shift=DOWN * 0.12), run_time=0.8)
        ...
        finish_to(self, TARGET_DURATIONS_V3["FirstGoldPmfCurveFull"])


class CumulativeCdfCurveFull(Scene):
    def construct(self) -> None:
        set_scene_background(self)
        title = scene_title("第三条：抽到这里，共有多少人出过金？")
        self.play(FadeIn(title, shift=DOWN * 0.12), run_time=0.8)
        ...
        finish_to(self, TARGET_DURATIONS_V3["CumulativeCdfCurveFull"])


class CompactCurveRelationship(Scene):
    def construct(self) -> None:
        set_scene_background(self)
        title = scene_title("三条曲线放在一起，分别记录什么？")
        self.play(FadeIn(title, shift=DOWN * 0.12), run_time=0.8)
        ...
        finish_to(self, TARGET_DURATIONS_V3["CompactCurveRelationship"])


class HardPityCurveDashboard(Scene):
    def construct(self) -> None:
        set_scene_background(self)
        title = scene_title("加入90抽硬保底，三条曲线会怎样变化？")
        self.play(FadeIn(title, shift=DOWN * 0.12), run_time=0.8)
        ...
        finish_to(self, TARGET_DURATIONS_V3["HardPityCurveDashboard"])


class IntegratedRateVsHardPity(Scene):
    def construct(self) -> None:
        set_scene_background(self)
        title = scene_title("1.6%还能告诉我们什么？", "比较平均五星间隔")
        self.play(FadeIn(title, shift=DOWN * 0.12), run_time=0.8)
        ...
        finish_to(self, TARGET_DURATIONS_V3["IntegratedRateVsHardPity"])


class NonUniquePerPullCurves(Scene):
    def construct(self) -> None:
        set_scene_background(self)
        title = scene_title("相同公开数字，可以对应不同的逐抽曲线")
        self.play(FadeIn(title, shift=DOWN * 0.12), run_time=0.8)
        ...
        finish_to(self, TARGET_DURATIONS_V3["NonUniquePerPullCurves"])


class FiveStarThenUpIdentityV3(Scene):
    def construct(self) -> None:
        set_scene_background(self)
        title = scene_title("出了五星以后，还要判断是不是当期UP")
        self.play(FadeIn(title, shift=DOWN * 0.12), run_time=0.8)
        ...
        five_star = result_card("五星出现", GOLD, "五星计数归零", 2.20).scale(0.78)
        ...
        self.play(FadeIn(five_star, shift=UP * 0.12), FadeIn(identity_heading), run_time=1.0)
        hold_to(self, 10.8)

        normal_state = pill("当前不是大保底", MUTED, 2.50).move_to(RIGHT_CENTER + UP * 1.20)
        fork = Dot(RIGHT_CENTER + LEFT * 1.22 + UP * 0.08, radius=0.075, color=GOLD)
        up = result_card("当期UP", PINK, "基础50%", 1.72).scale(0.58)
        up.move_to(RIGHT_CENTER + RIGHT * 0.92 + UP * 0.18)
        off = result_card("非UP五星", GRAY, "俗称歪了", 1.72).scale(0.58)
        ...
        guarantee = pill("歪了以后：下一颗五星必为当期UP", GOLD, 3.80)
        guarantee.move_to(RIGHT_CENTER + DOWN * 2.30)
        state_note = VGroup(
            pill("记录1：五星计数", BLUE, 2.06),
            pill("记录2：是否大保底", GOLD, 2.35),
        ).arrange(RIGHT, buff=0.22)
        state_note.move_to(LEFT_CENTER + DOWN * 2.35)
        # Peak occupancy: left pity timeline and two state records; right 50/50 outcomes plus guarantee state.
        ...
        self.play(ReplacementTransform(guarantee, warning), run_time=0.8)
        finish_to(self, TARGET_DURATIONS_V3["FiveStarThenUpIdentityV3"])


class UpWaitingTwoPeaksV3(Scene):
    def construct(self) -> None:
        set_scene_background(self)
        title = scene_title(
            "第一次拿到当期UP，为什么会有两个峰？",
        ...
        first_component[:90] = 0.5 * TEACHING_WAIT_PMF
        convolution = np.convolve(TEACHING_WAIT_PMF, TEACHING_WAIT_PMF)
        ...
        finish_to(self, TARGET_DURATIONS_V3["UpWaitingTwoPeaksV3"])


class CapturingRadianceBasicsV3(Scene):
    def construct(self) -> None:
        set_scene_background(self)
        title = scene_title("非大保底五星，多了一项UP判定")
        self.play(FadeIn(title, shift=DOWN * 0.12), run_time=0.8)

        visual_box = left_panel(edge=PINK, fill="#0A1525")
        info_box = right_panel(edge=PINK, fill="#0A1525")
        entering = result_card("已经出五星", GOLD, "当前不是大保底", 2.18).scale(0.78)
        ...
        ordinary = pill("50/50基础判定", MUTED, 2.22)
        ...
        up_result = result_card("当期UP", PINK, None, 1.70).scale(0.64)
        up_result.move_to(LEFT_CENTER + RIGHT * 3.05 + UP * 0.78)
        off_result = result_card("非UP五星", GRAY, None, 1.70).scale(0.64)
        ...
        facts = VGroup(
            ...
            stat_card("计入机制后的UP综合概率", r"55\%", MINT, "分母：已出五星且非大保底", 3.82, 1.62),
        ).arrange(DOWN, buff=0.24).move_to(RIGHT_CENTER + UP * 0.18)
        fact_note = cn("55%不是单抽直接获得当期UP的概率", 0.27, MUTED)
        fit_inside(fact_note, 3.66, 0.60)
        fact_note.move_to(RIGHT_CENTER + DOWN * 2.02)
        self.play(FadeOut(intro_state), FadeIn(facts), FadeIn(fact_note), run_time=1.3)
        hold_to(self, 26.6)
        # Peak occupancy: one left decision tree and two denominator-specific fact cards on the right.
        self.play(
            Indicate(facts[0], color=PINK),
            Indicate(facts[1], color=MINT),
            run_time=1.5,
        )
        finish_to(self, TARGET_DURATIONS_V3["CapturingRadianceBasicsV3"])


class CapturingRadianceHistoryV3(Scene):
    def construct(self) -> None:
        set_scene_background(self)
        title = scene_title("捕获明光还会参考此前的非UP结果")
        self.play(FadeIn(title, shift=DOWN * 0.12), run_time=0.8)
        ...
        for index, cycle in enumerate(cycles):
            ...
            self.play(FadeIn(cycle, shift=RIGHT * 0.12), ReplacementTransform(current_state, next_state), run_time=1.0)
        ...
        self.play(ReplacementTransform(current_state, trigger_state), FadeIn(next_star), run_time=1.2)
        hold_to(self, 21.3)
        ...
        finish_to(self, TARGET_DURATIONS_V3["CapturingRadianceHistoryV3"])


class ProbabilityStateConclusionV3(Scene):
    def construct(self) -> None:
        set_scene_background(self)
        title = scene_title("最后，把三种概率和账号状态放回同一张图")
        self.play(FadeIn(title, shift=DOWN * 0.12), run_time=0.8)
        ...
        self.play(FadeIn(numbers_card), ReplacementTransform(all_state, number_state), run_time=1.1)
        hold_to(self, 23.6)
        ...
        final_state = info_state(
            ...
            [
                ("先说清楚：你问的是哪一种概率？", GOLD),
                ("这一抽、首金分布，还是累计出金率？", INK),
            ],
            accent=GOLD,
        )
        # Peak occupancy: the complete left 2x2 recap and one final question state on the right.
        ...
        finish_to(self, TARGET_DURATIONS_V3["ProbabilityStateConclusionV3"])
