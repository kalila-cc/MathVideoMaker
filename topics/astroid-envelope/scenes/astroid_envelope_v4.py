...
class VariablesMatter(Scene):
    def construct(self) -> None:
        self.camera.background_color = BG
        title = cn("只看这一瞬间", 0.52).to_edge(UP)
        subtitle = cn("每个符号都对应画面里的一个真实量", 0.34, MUTED).next_to(title, DOWN, buff=0.14)
...
class LineEquationNatural(Scene):
    def construct(self) -> None:
        self.camera.background_color = BG
        title = cn("把这一根棍子写成直线", 0.5).to_edge(UP)
...
class EnvelopeIdea(Scene):
    def construct(self) -> None:
        self.camera.background_color = BG
        title = cn("边界点还有第二个条件", 0.5).to_edge(UP)
        self.play(FadeIn(title, shift=DOWN * 0.18), run_time=0.8)
        self.play(Create(corner()), run_time=0.8)
...
class DerivativeStep(Scene):
    def construct(self) -> None:
        self.camera.background_color = BG
        title = cn("导数式从这里来", 0.5).to_edge(UP)
        self.play(FadeIn(title, shift=DOWN * 0.18), run_time=0.8)
        self.play(Create(corner()), run_time=0.8)
...
class SolveParameter(Scene):
    def construct(self) -> None:
        self.camera.background_color = BG
        title = cn("两个条件联立，参数式就出来了", 0.48).to_edge(UP)
        self.play(FadeIn(title, shift=DOWN * 0.18), run_time=0.8)
