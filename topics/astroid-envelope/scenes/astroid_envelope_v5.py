...
class CoverFrame(Scene):
    def construct(self) -> None:
        ...
        title = cn("一根滑落的棍子，为什么会画出星形线？", 0.58).to_edge(UP, buff=0.72)
        subtitle = cn("从墙角里的日常动作，看见一条四尖角的包络线", 0.34, MUTED).next_to(title, DOWN, buff=0.2)

        center = LEFT * 3.35 + DOWN * 0.35
        axes = astroid_axes(center=center, radius=1.72).set_opacity(0.72)
        curve = full_astroid(center=center, radius=1.72, width=11)
        glow = full_astroid(center=center, radius=1.72, width=24).set_stroke(color=AMBER, opacity=0.16)
        stars = VGroup(*[Dot(astroid_point(t, center, 1.72), color=AMBER, radius=0.065) for t in [0, PI / 2, PI, 3 * PI / 2]])

        mini_origin = RIGHT * 2.05 + DOWN * 1.95
        ...
        self.add(card, title, subtitle, glow, axes, curve, stars, scenario, formula)
        self.wait(3.55)
