...
def flow_cue(start: np.ndarray, end: np.ndarray, color: str = AMBER, width: float = 4.0) -> Arrow:
    return Arrow(
        start,
        end,
        buff=0,
...
def capstan_visual() -> VGroup:
    post = VGroup(
        Circle(radius=1.08, color="#52657E", stroke_width=6, fill_color="#1B2A3D", fill_opacity=1),
        Circle(radius=0.78, color="#70839C", stroke_width=2, fill_color="#26384F", fill_opacity=0.72).shift(UP * 0.05),
        Circle(radius=0.42, color="#9AAABE", stroke_width=1.4, fill_color="#334860", fill_opacity=0.58).shift(UP * 0.10),
    )
    shadow = Arc(radius=1.48, start_angle=-0.82 * PI, angle=1.55 * TAU, color="#7B5722", stroke_width=19, stroke_opacity=0.42)
    wrap = Arc(radius=1.42, start_angle=-0.82 * PI, angle=1.55 * TAU, color=AMBER, stroke_width=12)
    start = wrap.point_from_proportion(0)
    end = wrap.point_from_proportion(1)
    light_tail = Line(LEFT * 4.85 + DOWN * 0.92, start, color=AMBER, stroke_width=12)
    heavy_tail = Line(end, RIGHT * 4.85 + UP * 0.96, color=AMBER, stroke_width=12)
    rope = VGroup(shadow, light_tail, wrap, heavy_tail)
    contact_band = Arc(radius=1.57, start_angle=-0.69 * PI, angle=1.34 * TAU, color=MINT, stroke_width=4, stroke_opacity=0.72)
    flow = VGroup(
        flow_cue(wrap.point_from_proportion(0.10), wrap.point_from_proportion(0.16), AMBER, 4.0),
        flow_cue(wrap.point_from_proportion(0.38), wrap.point_from_proportion(0.44), AMBER, 4.0),
        flow_cue(wrap.point_from_proportion(0.68), wrap.point_from_proportion(0.74), AMBER, 4.0),
    )
    light_arrow = Arrow(LEFT * 4.55 + DOWN * 1.28, LEFT * 3.25 + DOWN * 1.05, buff=0, color=ROSE, stroke_width=5)
    heavy_arrow = Arrow(RIGHT * 3.20 + UP * 1.22, RIGHT * 4.55 + UP * 1.05, buff=0, color=ROSE, stroke_width=7)
    labels = VGroup(
    ...
    segment = Arc(radius=1.78, start_angle=0.18 * PI, angle=0.24 * PI, color=MINT, stroke_width=6)
    dtheta = MathTex(r"d\theta", color=MINT).scale(0.55).next_to(segment, UP, buff=0.05)
    normal = Arrow(UP * 1.88 + LEFT * 0.24, UP * 1.10 + LEFT * 0.10, buff=0, color=BLUE, stroke_width=5)
    n_label = MathTex("dN", color=BLUE).scale(0.52).next_to(normal, LEFT, buff=0.08)
    cue = cn("缆绳贴着柱面绕行", 0.27, MINT).next_to(post, DOWN, buff=0.18)
    return VGroup(post, rope, contact_band, flow, light_arrow, heavy_arrow, labels, segment, dtheta, normal, n_label, cue)
...
class FrictionAmplification(Scene):
    ...
    def opening_books(self) -> None:
        ...
        wait_until(self, 4.60)
        self.play(LaggedStart(*[Indicate(badge, scale_factor=1.08) for badge in badges], lag_ratio=0.16), run_time=1.1)
        wait_until(self, 8.04)
        page_pulls = [
            page.animate.shift((LEFT if index % 2 else RIGHT) * 0.08)
            for index, page in enumerate(books[1])
        ]
        self.play(arrows.animate.scale(1.13), *page_pulls, rate_func=there_and_back, run_time=1.35)
        wait_until(self, 14.25)
        self.play(question.animate.set_color(ROSE).scale(1.04), rate_func=there_and_back, run_time=0.9)
    ...
    def single_page(self) -> None:
        ...
        wait_until(self, 23.53)
        self.play(Indicate(diagram[1], color=AMBER, scale_factor=1.03), run_time=0.9)
        wait_until(self, 29.03)
        flash = diagram[1].copy().set_stroke(ROSE, width=12)
        self.play(
            ShowPassingFlash(flash, time_width=0.75),
            diagram[2].animate.set_color("#FF8AA0"),
            diagram[3].animate.set_color("#FF8AA0"),
            run_time=1.2,
        )
        wait_until(self, 35.04)
    ...
    def normal_force_formula(self) -> None:
        ...
        self.current = self.replace_scene(self.current, title, body)
        wait_until(self, 40.59)
        self.play(Indicate(split[2], color=BLUE, scale_factor=1.18), run_time=0.9)
        wait_until(self, 43.71)
        self.play(Indicate(formula[2], color=AMBER, scale_factor=1.08), run_time=1.1)
        wait_until(self, 50.15)
        self.play(Indicate(formula[3][0], color=BLUE, scale_factor=1.12), run_time=0.9)
        wait_until(self, 57.88)
        self.play(ShowPassingFlash(bridge.copy().set_stroke(MINT, width=8), time_width=0.65), run_time=0.9)
    ...
    def many_layers(self) -> None:
        ...
        body = VGroup(stack, counter, bar1, bar2)
        self.current = self.replace_scene(self.current, title, body)
        wait_until(self, 67.23)
        self.play(LaggedStart(*[FadeIn(mob, scale=1.05) for mob in arrows], lag_ratio=0.05), run_time=1.4)
        self.current.add(arrows)
        wait_until(self, 73.02)
        self.play(Indicate(counter, color=AMBER, scale_factor=1.04), run_time=0.9)
        wait_until(self, 78.20)
    ...
    def page_count_trend(self) -> None:
        ...
        self.current = self.replace_scene(self.current, title, body)
        wait_until(self, 87.91)
        self.play(Create(plot[4]), run_time=1.5)
        marker = Dot(plot[4].get_start(), color=ROSE, radius=0.085)
        self.add(marker)
        self.current.add(marker)
        self.play(MoveAlongPath(marker, plot[4]), run_time=2.2)
        wait_until(self, 93.71)
        self.play(LaggedStart(*[FadeIn(item, shift=UP * 0.08) for item in feedback], lag_ratio=0.08), run_time=1.0)
        wait_until(self, 99.0)
        self.play(Indicate(feedback, color=AMBER, scale_factor=1.02), run_time=1.1)
    ...
    def capstan_setup(self) -> None:
        ...
        self.current = self.replace_scene(self.current, title, body)
        wait_until(self, 109.24)
        self.play(ShowPassingFlash(visual[1][2].copy().set_stroke(AMBER, width=17), time_width=0.55), run_time=1.4)
        wait_until(self, 112.04)
        self.play(LaggedStart(*[Indicate(arrow, scale_factor=1.12) for arrow in visual[3]], lag_ratio=0.18), run_time=1.5)
        wait_until(self, 118.0)
        self.play(Indicate(visual[9], color=BLUE, scale_factor=1.16), Indicate(visual[7], color=MINT, scale_factor=1.12), run_time=1.2)
    ...
    def capstan_equation(self) -> None:
        ...
        self.current = self.replace_scene(self.current, title, body)
        wait_until(self, 129.0)
        self.play(ShowPassingFlash(capstan[1][2].copy().set_stroke(AMBER, width=13), time_width=0.55), run_time=1.2)
        wait_until(self, 134.0)
        self.play(Indicate(panel[2], color=AMBER, scale_factor=1.05), run_time=1.2)
        wait_until(self, 139.77)
    ...
    def same_mechanism(self) -> None:
        ...
        self.current = self.replace_scene(self.current, title, body)
        wait_until(self, 147.30)
        self.play(Indicate(books, color=BLUE, scale_factor=1.03), Indicate(capstan, color=AMBER, scale_factor=1.03), run_time=1.2)
        wait_until(self, 152.52)
