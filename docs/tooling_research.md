...
| Remotion | React 组件化视频、字幕/模板/批量生成能力强，适合统一标题、章节、字幕、后期包装和系列视觉系统 | 不是数学动画专用；复杂几何要自己造轮子；如果完全替代 Manim，会削弱公式和几何构造的精确性 | 系列包装、批量短视频、数据驱动模板、Manim 片段的后期编排层 | 第二阶段包装层 |
...
Remotion 已可作为第二阶段工具引入，但不建议立刻替代 Manim。当前更稳的方式是混合管线：

1. Manim 继续负责几何构造、公式变形、曲线和控制点的精确动画。
2. 低清分段预览阶段仍以 Manim 片段为主，只重渲染被反馈的 Scene。
3. 当内容顺序、旁白和符号标注基本稳定后，再建立 Remotion composition，把 Manim 片段作为视频素材导入。
4. Remotion 负责统一界面包装：章节片头、细粒度字幕、边栏提示、公式 callout、轻量转场、统一噪声纹理、品牌片尾和横竖版重排。
5. 最终整片可由 Remotion 输出，也可以只输出透明/非透明包装层，再用 FFmpeg 与 Manim 主画面合成。

对贝塞尔曲线这类视频，Remotion 最值得优先优化的是“内容外壳”，不是数学核心动画本身：统一视觉语言、章节切换、字幕层级、关键术语标牌、预览进度条和片头片尾。这能提升质感，同时避免把已稳定的 Manim 推导镜头重写一遍。
