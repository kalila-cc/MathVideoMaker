...
4. 复查高清信息峰值帧：左右信息卡、公式栈、字幕安全区和品牌转场都必须使用当前高清产物抽帧，不沿用低清截图。
5. 用当前旁白音轨合成最终 MP4；若前面有静音片头，用实测片头时长给音频加延迟，并生成同样后移的最终 SRT。
6. 从独立封面帧或封面 Scene 生成 cover JPG，并写回 `data/videos.json`；不要截 ChatGPT 片头作为主题封面。
7. 更新 metadata：标题、简介、标签、状态、优先级、封面、总时长和章节；章节时间必须来自这次高清片段和最终 MP4，而不是旧低清预览。
8. 验证最终 MP4 的分辨率、帧率、时长和音轨，并执行一次完整解码。
9. 预览网页确认只展示最终成片，章节跳转和封面正常。
10. dry-run 清理旧产物，确认清单无误后再执行删除。
11. 删除旧视频后再次验证 `data/videos.json` 和图库 API。
...
- 卡片：对每个高密度卡片至少抽取进入帧、信息峰值帧和结论帧；内部对象不得触碰卡片四边，已完成的过程应通过替换或淡出为结论让位。
- 高清视觉：低清预览确认过的关键帧必须从最终高清 MP4 再抽一次，防止字体度量、字幕缩放和重渲染版本差异造成回归。
- 解码：使用 `ffmpeg -v error -i <final.mp4> -f null NUL` 完整跑到结尾，退出码必须为 0。
